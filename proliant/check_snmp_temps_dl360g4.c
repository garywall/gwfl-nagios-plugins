/* $Id: check_snmp_temps_dl360g4.c,v 1.3 2005/11/03 20:13:57 gary Exp $
 *
 * check_snmp_temps_dl360g4.c -- check system temperatures via SNMP
 *
...
#define PROGNAME "check_snmp_temps_dl360g4"
#define SUMMARY "Check the system temperatures on a DL360 G4 using SNMP."
...
float thr[] = {63.0, 85.0, 85.0, 48.0, 41.0};
...
int main(int argc, char **argv)
{
	...
	if (perfdata) {
		...
		while (p != NULL) {
			...
			if (strstr(names[p->index], "I/O zone"))
			...
			else if (strstr(names[p->index], "PSU bay"))
				printf(" PSUbay=%.1f", p->data);
			else if (strstr(names[p->index], "System board"))
				printf(" SYSboard=%.1f", p->data);
