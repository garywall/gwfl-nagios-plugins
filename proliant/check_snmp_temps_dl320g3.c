/* $Id: check_snmp_temps_dl320g3.c,v 1.1 2006/05/25 10:03:38 gary Exp $
 *
 * check_snmp_temps_dl320g3.c -- check system temperatures via SNMP
 *
...
#define PROGNAME "check_snmp_temps_dl320g3"
#define SUMMARY "Check the system temperatures on a DL320 G3 using SNMP."

#define OPTIONS "-H <ip_address> [-C community] [-p]\n\nIf you want this plugin to return performance data for Nagios, use the `-p` option."
...
int parse_args(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hh:H:C:w:p")) != -1) {
		switch (c) {
			case 'h':
				print_help();
				exit(UNKNOWN);
